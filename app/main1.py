...
import requests
import json
from oauth import get_flow, authenticate_user, handle_authorization, get_credentials, SCOPES
...
def create_streamlit_app(llm, clean_text):
    ...
    if 'code' in query_params:
        ...
        st.write(f"Handling authorization with code: {code}")
        if handle_authorization(flow, code):
            st.write("Authorization successful, getting credentials")
            credentials = get_credentials()
            if credentials:
                st.write(f"Credentials obtained: {credentials}")
                st.session_state.token = credentials.token
                st.write(f"Token set in session state: {st.session_state.token}")
                user_info = get_user_info(st.session_state.token)
                st.write(f"User info retrieved: {user_info}")
                st.session_state.user_email = user_info.get('email')
                st.write(f"User email set in session state: {st.session_state.user_email}")
                st.success("Logged in successfully!")
                st.write("Rerunning Streamlit app")
                st.rerun()
            else:
                st.error("Failed to get credentials after authorization.")
                st.write("Credentials not obtained after authorization")
        else:
            st.error("Authorization failed. Please try again.")
            st.write("Authorization handling failed")
        ...
        if send_email_button:
            if recipient_email and st.session_state.email_body and st.session_state.subject:
                if "@" not in recipient_email or "." not in recipient_email:
                    ...
                    try:
                        result = send_email(
                            get_credentials(),  # Pass credentials
